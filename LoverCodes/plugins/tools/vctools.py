...
from LoverCodes import app, Userbot
...
from pytgcalls.exceptions import (NoActiveGroupCall, CallBusy)

@app.on_message(filters.command(["vcinfo"], ["/", "!"]))
async def strcall(client, message):
    assistant = await group_assistant(Lover, message.chat.id)
    try:
        await assistant.play(message.chat.id, MediaStream("./LoverCodes/assets/call.mp3"))
