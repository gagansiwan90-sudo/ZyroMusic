...
from LoverCodes import YouTube, app
from LoverCodes.core.call import Lover
from LoverCodes.misc import SUDOERS, db
...
from LoverCodes.utils.database import get_assistant
from LoverCodes.utils.decorators.language import languageCB
from LoverCodes.utils.formatters import seconds_to_min
from LoverCodes.utils.inline import close_markup, stream_markup, stream_markup_timer
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Lover.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Lover.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Lover.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Lover.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
